...
class TFT_HX8357GLUE : public mcufriend_blue_pill
{
    public:
        TFT_HX8357GLUE() {}
        void begin(void) {
		...
		void init(void)
        {
			...
			_ID = mcufriend_blue_pill::readID();
            if (_ID == 0x00D3 || _ID == 0xD3D3)
			   _ID = 0x9486;
            mcufriend_blue_pill::begin(_ID);
            mcufriend_blue_pill::setRotation(1);
            _first = true;
        }
        void setWindow(int16_t x0, int16_t y0, int16_t x1, int16_t y1)
        {
            mcufriend_blue_pill::setAddrWindow(x0, y0, x1, y1);
            _first = true;
        }

        void pushColors(uint8_t *data, uint16_t len)
        {
            mcufriend_blue_pill::pushColors((uint8_t*)data, len, _first);
            _first = false;
        }

        void pushColors(uint16_t *data, uint8_t len)
        {
            mcufriend_blue_pill::pushColors((uint16_t*)data, len, _first);
            _first = false;
        }

        void pushColors(uint16_t *data, uint16_t len, int first)
        {
            mcufriend_blue_pill::pushColors((uint16_t*)data, len, first);
        }

        void pushColor(uint16_t color)
        {
            uint16_t c = color;
			mcufriend_blue_pill::pushColors(&c, 1, _first);
            _first = false;
        }
        ...
        void setCursor(int16_t x, int16_t y, uint8_t idx)
        {
            ...
            mcufriend_blue_pill::setFont(f);
//            mcufriend_blue_pill::setCursor(x, y + f->glyph->height);
        ...
        {
            _font = font;
            mcufriend_blue_pill::setFont(Fonts[_font]);
        }
        ...
        int16_t drawString(char *string, int16_t poX, int16_t poY, int16_t idx)
        {
            ...
            mcufriend_blue_pill::setCursor(poX, poY + h);
            print(string);
            return w;
        }
